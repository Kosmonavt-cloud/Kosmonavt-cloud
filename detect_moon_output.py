...
def process_data(file_name, time_bins_top3, sum_psd_excluding_top3_amp_atten, on_off_psd, average_low_value, range_low_values, tr_time, tr_times_filt):
    ...
    for j in range(len(on_off_psd)):
        ...
        if triggers_psd[0] < len(time_bins_top3):
            ...
            if abs(time_bins_top3[triggers_psd[1]] - time_bins_top3[triggers_psd[0]]) < 1000:
                if (index < len(sum_psd_excluding_top3_amp_atten) and 
                    ...
                    abs(time_bins_top3[triggers_psd[1]] - time_bins_top3[triggers_psd[0]]) > 400):
...
#Get data------------------------------------------------------------------------------------
cat_directory = './space_apps_2024_seismic_detection/data/lunar/test/catalogs/'
cat_file = cat_directory + 'apollo16_catalog_GradeB_final.csv'
...
for l in range(len(cat)):
    ...
    data_directory = './space_apps_2024_seismic_detection/data/lunar/test/data/S16_GradeB/'
...
detect_df.to_csv('./space_apps_2024_seismic_detection/data/lunar/detection_results_lunar_test_s16B.csv', index=False)
