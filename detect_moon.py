...
row = cat.iloc[40]           #modify number to choose event (6 - first detection)
...
for j in np.arange(0,len(on_off_psd)):
    ...
    if abs(time_bins_top3[triggers_psd[1]]-time_bins_top3[triggers_psd[0]]) < 1000:       #time period for when PSD sum is too short => does not flag
        if sum_psd_excluding_top3_amp_atten[index] > (average_low_value + range_low_values) and abs(time_bins_top3[triggers_psd[1]]-time_bins_top3[triggers_psd[0]]) > 400:
